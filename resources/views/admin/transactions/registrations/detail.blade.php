    ...
    <div class="row">
        <div class="row mb-3">
            <div class="col-lg-2 d-flex align-items-stretch">
                <a href="{{ route('transaction.registrations.show', request()->event_id) }}" class="btn btn-primary">
                        ...
                        <div class="d-flex flex-row p-2 w-100 overflow-auto" style="gap: 40px;">
                            <div class="d-flex flex-column bd-highlight" style="gap: 5px;">
                                ...
                                <div class="d-flex flex-row bd-highlight justify-content-evenly" style="gap: 5px;">
                                    ...
                                    <button type="button" class="btn btn-padding btn-vip btn-choose"
                                        {{ in_array('J8', $seatAlreadyBook->pluck('name')->toArray()) ? 'disabled' : '' }}>J8</button>
                                </div>
                                ...
                                <div class="d-flex flex-row bd-highlight justify-content-evenly" style="gap: 5px;">
                                    <button type="button" class="btn btn-padding btn-secondary btn-choose"
...
@section('script')
    ...
    <script>
        ...
        const listSeat = {!! json_encode($seats) !!}
        let seats = []
        ...
        $('.btn-choose').on('click', function() {
            ...
            const chooseSeat = listSeat.find((element) => element.name === value)

            if (seats.length === +limitSeat) {
                Swal.fire({
                    title: "Terjadi kesalahan",
                    text: `Maksimal kursi yang ditambahkan adalah ${limitSeat}`,
                    icon: "error",
                })
            ...
            const checkSeatAlreadyChoose = seats.find(element => element === +chooseSeat.id)
            if (checkSeatAlreadyChoose) {
                Swal.fire({
                    title: "Terjadi kesalahan",
                    text: `Kursi sudah ditambahkan ${value}`,
                    icon: "error",
                })
            ...
            if (chooseSeat) {
                seats.push(chooseSeat.id)

                $('.select2-elements').val(seats)
                $('.select2-elements').trigger('change')

                Swal.fire({
                    title: "Kursi ditambahkan",
                    text: `Nama kursi ${value} telah ditambahkan`,
                    icon: "info",
                })
            } else {
                Swal.fire({
                    title: "Terjadi kesalahan",
                    text: `Nama kursi ${value} tidak ditemukan`,
                    icon: "error",
                })
            }
        ...
        $('.select2-elements').on('select2:unselect', function(e) {
            const data = e.params.data;
            seats = seats.filter(element => element !== +data.id)
        })
    </script>
@endsection
